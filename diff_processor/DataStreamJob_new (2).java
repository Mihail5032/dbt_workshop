...
public class DataStreamJob {
    ...
    public void run(String[] args) throws Exception {
        ...
        // Этап 2: keyBy(txnKey) → TransactionProcessor (дедуп + десериализация + эмит)
        TransactionProcessor txnProcessor = new TransactionProcessor(icebergSchemas, pstSchemas);

        SingleOutputStreamOperator<RowData> processed = parsed
                .keyBy(TransactionBundle::getTxnKey)
                .process(txnProcessor)
                .setParallelism(15)
                .uid(UID_PROCESSOR);
