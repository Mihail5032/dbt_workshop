...
public class BootstrappingTransactionProcessor
    ...
    /** Локальный кеш флага "bootstrap завершён" — избегаем обращения к broadcast state на каждую запись */
    private transient boolean bootstrapDoneLocal = false;
    ...
    public void processElement(TransactionBundle bundle,
        ...
        // === 3) БЫСТРЫЙ ПУТЬ: bootstrap уже завершён (кешированный флаг) ===
        // После переключения — работаем как обычный TransactionProcessor,
        // без обращения к broadcast state, без буферов, без таймеров.
        if (bootstrapDoneLocal) {
            seenState.update(true);
            deserializeAndEmit(bundle.getPayload(), ctx, out);
            if (totalCount % LOG_INTERVAL == 0) {
                log.info("TXN_PROCESSOR_STATS: total={}, dupes={}, passed={}, subtask={}",
        ...
        // === 4) Проверяем broadcast state (только пока bootstrap не завершён) ===
        ...
        // === 5) Bootstrap только что завершился — переключаемся на быстрый путь ===
        bootstrapDoneLocal = true;
        log.info("TXN_PROCESSOR_FAST_PATH: switching to fast path, subtask={}",
                getRuntimeContext().getTaskInfo().getIndexOfThisSubtask());

        // Лениво флашим буфер этого ключа, потом сам bundle
        flushBuffer(ctx, out);
    ...
    public void processBroadcastElement(Boolean signal,
        ...
        bootstrapDoneLocal = true;
        log.info("BOOTSTRAP_END received, switching to fast path. subtask={}. " +
                 "Buffered Kafka-bundles will flush lazily or via timer.",
                getRuntimeContext().getTaskInfo().getIndexOfThisSubtask());
    }
