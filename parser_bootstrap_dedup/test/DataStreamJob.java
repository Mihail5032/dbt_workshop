...
public class DataStreamJob {
    ...
    public void run() throws Exception {
        ...
        // ТЕСТ: sourcedocument-sink отключён — side output повисает без потребителя,
        // Flink из-за этого не падает.
        ...
        // ======================== ТЕСТ: единственный sink в raw_bptransaction_test ========================
        // Все остальные RAW/PST sink-и (rawOnlyTableMap, полный проход по pstTableMap) отключены.
        // Side output-ы других сегментов просто повисают без потребителя — Flink из-за этого не падает.
        // Схемы для всех таблиц по-прежнему грузятся выше — они нужны IdocParser-у для сериализации.
        OutputTag<RowData> testSinkTag = StreamSideOutputTag.getTag("PST_BPTRANSACTION");
        DataStream<RowData> testSinkStream = processed.getSideOutput(testSinkTag);
        TableLoader testSinkLoader = TableLoader.fromCatalog(
                catalogLoader,
                TableIdentifier.of(SCHEMA, "raw_bptransaction_test"));

        FlinkSink.forRowData(testSinkStream)
                .tableLoader(testSinkLoader)
                .writeParallelism(2)
                .upsert(false)
                .uidPrefix("sink-raw_bptransaction_test")
                .set("write.target-file-size-bytes", "268435456")
                .append();

        env.execute("XML Parser TEST — writes only to raw_bptransaction_test");
