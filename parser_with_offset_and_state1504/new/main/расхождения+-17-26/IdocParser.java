...
public class IdocParser extends ProcessFunction<String, TransactionBundle> {
    ...
    // Расхождение #22: matnr (ALPHA-padded, 18) -> meins (базовая единица измерения из MARA)
    private final HashMap<String, String> maraMeinsMap = new HashMap<>();
    ...
    private Long lastSnapshotMara = -1L;
    private Long lastSnapshotTvarvc = -1L;
    private long lastTvarvcCheckTime = 0;
    private static final long TVARVC_CHECK_INTERVAL_MS = 600_000;
    ...
    private transient TableIdentifier tableIdentifierMara;
    ...
    public void open(org.apache.flink.api.common.functions.OpenContext openContext) throws Exception {
        ...
        // Задержка старта парсера, чтобы IcebergBootstrapSource успел прогреть seenState
        // до того, как из Kafka через парсер потекут данные. Пока мы спим —
        // KafkaSource упирается в backpressure и почти не вычитывает, bootstrap работает
        // параллельно и заполняет state. Значение в мс берём из properties, по умолчанию 5 мин.
        long startupDelayMs = config.getStartupDelayMs();
        if (startupDelayMs > 0) {
            log.info("PARSER_STARTUP_DELAY: sleeping {} ms before first record to let bootstrap warm up state, subtask={}",
                    startupDelayMs, getRuntimeContext().getTaskInfo().getIndexOfThisSubtask());
            Thread.sleep(startupDelayMs);
            log.info("PARSER_STARTUP_DELAY: woke up, starting parsing, subtask={}",
        ...
        this.tableIdentifierMara = TableIdentifier.of(SCHEMA_DICT, "mara_full_fm");
    ...
    public void processElement(String kafkaValue, Context ctx, Collector<TransactionBundle> out) {
        ...
        try (InputStream inputStream = CustomDecoder.decodeAndDecompress(kafkaValue)) {
            ...
            Set<String> droppedTxnKeys = Collections.emptySet();
            try {
                droppedTxnKeys = getDataRetailLineItemFromDictionaries(retailLineItems);
            } catch (Exception e) {
                log.error("Failed to retrieve dictionaries data for retaillineitem", e);
            }
            try {
                updateDictTvarvc();  //обновление справочника tvarvc для проверки CHK_FIXES
            } catch (Exception e) {
            ...
            for (Map.Entry<String, List<BaseTransactionKey>> entry : txnGroups.entrySet()) {
                ...
                // Расхождение #22: дроп всей транзакции, если хотя бы один itemid с '*UP*'
                // не удалось резолвить в EAN через marm_fm+mean_fm (ABAP: DELETE ct_transaction).
                if (droppedTxnKeys.contains(txnKey)) continue;
    ...
    private List<SegmentEntry> buildSegmentEntries(List<BaseTransactionKey> groupSegments,
        ...
        boolean chkFixesHas2125781 = chkFixesSet.contains("2125781");  //проверка как в ABAP: IF '2125781' IN lr_fixes
        Set<String> autoMDSet = chkFixesHas2125781 ? getAutoMDSet(groupSegments) : Collections.emptySet();   //получение автоуценок только если есть запись в CHK_FIXES
        boolean chkFixes2525942 = chkFixesSet.contains("2525942");    //проверка как в ABAP: IF '2525942' IN lr_fixes (стр. 292)
        boolean chkFixes2525942_2 = chkFixesSet.contains("2525942_2");  //проверка как в ABAP: IF '2525942_2' IN lr_fixes (стр. 298)
        ...
        for (BaseTransactionKey btk : groupSegments) {
            try {
                // Расхождение #14: SAP не пишет TransactionExtension с transactionTypeCode = null
                if (btk instanceof TransactionExtension && btk.getTransactionTypeCode() == null) {
                    continue;
                }
                ...
                if (btk instanceof Transaction) {
                    ...
                    if (pstSchema != null) {
                        ...
                        if ("2020".equals(retailLineItem.getRetailTypeCode())) {
                            // код 2020 отсутствует в SAP (recordqualifier <> 'S') — не пишем в таблицу
                        } else if (retailLineItem.getHasEan() == null || Boolean.TRUE.equals(retailLineItem.getHasEan())) {
                            String key = retailLineItem.createKeyLineItem();
                            pstRowData = retailLineItem.toRowDataPst(pstSchema, timestampDataXml, dateXml,
                                    autoMDSet.contains(key), chkFixes2525942, chkFixes2525942_2);
    ...
    /**
     * Расхождение #22 (ABAP: "Преобразуем позиции с упаковками"):
     * для RetailLineItem с itemid, содержащим 'UP' (напр. "123UP4"):
     *   - ищем в marm_fm+mean_fm запись с совпадающими matnr/umrez/meinh и lfnum='00001',
     *     имеющую непустой ean11;
     *   - если найдена: подменяем itemId=ean11, itemIdQualifier='1',
     *     salesUnitOfMeasure=MARA-MEINS (из mara_full_fm по matnr);
     *   - если не найдена (EAN пустой): транзакция ЦЕЛИКОМ выбрасывается из вывода
     *     (возвращённый Set содержит её txnKey; вызывающий пропускает out.collect).
     */
    private Set<String> getDataRetailLineItemFromDictionaries(List<RetailLineItem> retailLineItemList) {
        Set<String> droppedTxnKeys = Collections.emptySet();
        boolean isUpdate = false;
        for (RetailLineItem retailLineItem : retailLineItemList) {
            if (retailLineItem.getItemId() == null || !retailLineItem.getItemId().contains("UP")) continue;
            ...
            for (MarmFm marmFm : marmFmList) {
                if (!retailLineItem.getMatnrPadded().equals(marmFm.getMatnr())
                        || !retailLineItem.getUmrezVal().equals(marmFm.getUmrez())) continue;
                ...
                for (MeanFm meanFm : meanFmList) {
                    if (!marmFm.getMatnr().equals(meanFm.getMatnr())
                            || marmFm.getMeinh() == null
                            || !marmFm.getMeinh().equals(meanFm.getMeinh())
                            || meanFm.getLfnum() == null
                            || !"00001".equals(meanFm.getLfnum())) continue;

                    retailLineItem.setLfnum(meanFm.getLfnum());
                    retailLineItem.setEan11(meanFm.getEan11());
                    boolean hasEan = meanFm.getEan11() != null && !meanFm.getEan11().isEmpty();
                    retailLineItem.setHasEan(hasEan);
                    if (hasEan) {
                        // ABAP: <item>-itemid = ls_ean-ean11; <item>-itemidqualifier = '1'; <item>-salesuom = ls_ean-meins.
                        retailLineItem.setItemId(meanFm.getEan11());
                        retailLineItem.setItemIdQualifier("1");
                        // salesuom = MARA-MEINS (базовая единица измерения из mara_full_fm по matnr).
                        String meins = maraMeinsMap.get(retailLineItem.getMatnrPadded());
                        if (meins != null && !meins.isEmpty()) {
                            retailLineItem.setSalesUnitOfMeasure(meins);
                        }
                        resolved = true;
                    }
                }
            ...
            if (!resolved) {
                // ABAP: ZPOSDW/025 → CLEAR transnumber → DELETE ct_transaction
                if (droppedTxnKeys.isEmpty()) droppedTxnKeys = new HashSet<>();
                String txnKey = buildTxnKey(retailLineItem);
                if (droppedTxnKeys.add(txnKey)) {
                    log.warn("Dropping transaction {}: cannot resolve itemid='{}' to EAN (ZPOSDW/025)",
                            txnKey, retailLineItem.getItemId());
                }
            }
        }
        return droppedTxnKeys;
    }
    ...
    /**
     * Расхождение #22: загрузка mara_full_fm для получения базовой единицы измерения (meins) по matnr.
     * ABAP: <item>-salesuom = ls_ean-meins, где meins = MARA-MEINS.
     * marm_fm содержит только meinh (альтернативная ЕИ), поэтому meins тянем из MARA напрямую.
     */
    public void updateDictMara() {
        try (TableLoader loader = TableLoader.fromCatalog(catalogLoaderDict, tableIdentifierMara)) {
            loader.open();
            Table table = loader.loadTable();
            Snapshot currentSnapshot = table.currentSnapshot();
            long snapshotId = currentSnapshot.snapshotId();
            if (lastSnapshotMara != snapshotId) {
                try (CloseableIterable<Record> records = IcebergGenerics.read(table).build()) {
                    maraMeinsMap.clear();
                    for (Record record : records) {
                        String matnr = (String) record.getField("matnr");
                        String meins = (String) record.getField("meins");
                        if (matnr == null || meins == null || meins.isEmpty()) continue;
                        // matnr в mara_full_fm обычно уже в ALPHA-формате (18 символов с ведущими нулями),
                        // поэтому кладём как есть — ключ совпадёт с retailLineItem.getMatnrPadded().
                        maraMeinsMap.put(matnr, meins);
                    }
                }
                lastSnapshotMara = snapshotId;
                log.info("Dictionary 'mara_full_fm' updated from snapshot " + snapshotId
                        + ", size=" + maraMeinsMap.size());
            }
        } catch (Exception e) {
            log.warn("Error updating dictionary 'mara_full_fm': ", e);
        }
    }

    public void updateDictTvarvc() {
        long now = System.currentTimeMillis();
        if (now - lastTvarvcCheckTime < TVARVC_CHECK_INTERVAL_MS) return;
        lastTvarvcCheckTime = now;
        try (TableLoader loader = TableLoader.fromCatalog(catalogLoaderDict, tableIdentifierTvarvc)) {
            loader.open();
            Table table = loader.loadTable();
            Snapshot currentSnapshot = table.currentSnapshot();
            long snapshotId = currentSnapshot.snapshotId();
            if (lastSnapshotTvarvc != snapshotId) {
                try (CloseableIterable<Record> records = IcebergGenerics.read(table).build()) {
                    chkFixesSet.clear();
                    for (Record record : records) {
                        String name = (String) record.getField("name");
                        String type = (String) record.getField("type");
                        if ("CHK_FIXES".equals(name) && "S".equals(type)) {
                            String low = (String) record.getField("low");
                            if (low != null) {
                                chkFixesSet.add(low);
                            }
                        }
                ...
                lastSnapshotTvarvc = snapshotId;
                log.info("Dictionary 'tvarvc' (CHK_FIXES) updated from snapshot " + snapshotId
                        + ", entries: " + chkFixesSet.size());
            }
        } catch (Exception e) {
