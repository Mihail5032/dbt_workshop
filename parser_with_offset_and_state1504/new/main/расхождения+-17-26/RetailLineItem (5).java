...
public class RetailLineItem extends BaseTransactionKey {
    ...
    public RowData toRowDataPst(Schema pstSchema, TimestampData timestampDataXml, LocalDate dateXml,
        ...
        // SAP хранит WORKSTATIONID как CHAR(10) с ведущими нулями ('0000001000'), в ABAP сравнивается
        // именно с паддингованной константой. В IDoc XML касса приходит БЕЗ паддинга ('1000', '70', '3')
        // и в таком виде ложится в Iceberg — поэтому прямое сравнение с '0000001000' не срабатывало
        // ни для одной записи (Расхождение #2). Сравниваем с игнором ведущих нулей: корректно работает
        // и для '1000', и для '0000001000' (на случай, если где-то в пайплайне встретится паддинг).
        if (chkFixes2525942 && isWorkstationId(workstationId, "1000")) {
    ...
    /**
     * Сравнение WORKSTATIONID с игнором ведущих нулей (без аллокаций).
     * В SAP поле CHAR(10) с паддингом нулями, в IDoc XML приходит без паддинга.
     * Возвращает true, если после снятия ведущих нулей ws == target.
     *
     * Производительность: два счётчика указателей, без new String / substring / StringBuilder.
     * Вызывается только если уже прошли chkFixes2525942 (редкий случай).
     */
    private static boolean isWorkstationId(String ws, String target) {
        if (ws == null || target == null) return false;
        int wsStart = 0, wsEnd = ws.length();
        while (wsStart < wsEnd && ws.charAt(wsStart) == '0') wsStart++;
        int tStart = 0, tEnd = target.length();
        while (tStart < tEnd && target.charAt(tStart) == '0') tStart++;
        if (wsEnd - wsStart != tEnd - tStart) return false;
        while (wsStart < wsEnd) {
            if (ws.charAt(wsStart++) != target.charAt(tStart++)) return false;
        }
        return true;
    }
