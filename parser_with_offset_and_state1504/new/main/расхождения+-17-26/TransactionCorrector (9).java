...
public class TransactionCorrector implements Serializable {
    ...
    public List<BaseTransactionKey> performCorrection(IDocWrapper doc) {
        ...
        correctBusinessDayDate(postrCreateMultip.getTransactions(), corrBaseTransactionKeyList);  // Расхождение #25
    ...
    /**
     * Расхождение #25: подмена businessdaydate на дату-часть enddatetimestamp
     * для транзакций с transactiontypecode LIKE '11%'.
     *
     * ABAP (TVARVC.ZGK_BUSDAY_CORRECT='X', применяется на входе — до TLOG*):
     *   LOOP AT ct_transaction ASSIGNING <t> WHERE transtypecode CP '11*'.
     *     CHECK <t>-endtimestamp(8) <> <t>-businessdaydate.
     *     <t>-businessdaydate = <t>-endtimestamp(8).
     *
     * SAP подменяет дату до записи во все TLOG-таблицы, поэтому парсер применяет
     * коррекцию ко ВСЕМ связанным сегментам (FM, line items, extensions, tenders,
     * totals, discounts), которые объединены бизнес-ключом
     * (retailstoreid|businessdaydate|workstationid|transactionsequencenumber).
     * После коррекции buildTxnKey и rtl_txn_rk автоматически пересчитываются
     * от новой даты.
     *
     * Производительность:
     *   1) Первый проход идёт только по списку Transaction (обычно 1–3 шт на IDOC),
     *      никаких аллокаций до первой найденной "11*"-транзакции с несовпадением.
     *   2) Второй проход по всем сегментам выполняется ТОЛЬКО если коррекция нужна.
     *      В подавляющем большинстве кейсов (чековые 10xx и уже согласованные 11xx)
     *      метод завершается после первого цикла без HashMap и без обхода сегментов.
     */
    private void correctBusinessDayDate(List<Transaction> transactions,
                                        List<BaseTransactionKey> allSegments) {
        if (transactions == null || transactions.isEmpty()) return;

        Map<String, String> corrections = null;
        for (Transaction t : transactions) {
            String type = t.getTransactionTypeCode();
            if (type == null || !type.startsWith("11")) continue;
            String end = t.getEndDateTimestamp();
            if (end == null || end.length() < 8) continue;
            String newBdd = end.substring(0, 8);
            String oldBdd = t.getBusinessDayDate();
            if (newBdd.equals(oldBdd)) continue;

            if (corrections == null) corrections = new HashMap<>(4);
            corrections.put(buildBusdayKey(t.getRetailStoreId(), oldBdd,
                            t.getWorkstationId(), t.getTransactionSequenceNumber()),
                    newBdd);
            log.info("BUSDAY_CORRECT (#25): txn " + t.getRetailStoreId() + "|" + oldBdd
                    + "|" + t.getWorkstationId() + "|" + t.getTransactionSequenceNumber()
                    + " -> businessdaydate=" + newBdd);
        }
        if (corrections == null) return;

        for (BaseTransactionKey btk : allSegments) {
            String newBdd = corrections.get(buildBusdayKey(
                    btk.getRetailStoreId(), btk.getBusinessDayDate(),
                    btk.getWorkstationId(), btk.getTransactionSequenceNumber()));
            if (newBdd != null) {
                btk.setBusinessDayDate(newBdd);
            }
        }
    }

    private static String buildBusdayKey(String store, String bdd, String ws, String tsn) {
        StringBuilder sb = new StringBuilder(48);
        sb.append(store == null ? "" : store).append('|')
          .append(bdd   == null ? "" : bdd).append('|')
          .append(ws    == null ? "" : ws).append('|')
          .append(tsn   == null ? "" : tsn);
        return sb.toString();
    }
    ...
    private void updateCancelledRetailLineItem(PostrCreateMultip postrCreateMultip) {
        ...
        if (!retailLineItemsCancel.isEmpty()) {
            for (LineItemExtension lineItemExtension : postrCreateMultip.getLineItemExtensions()) {  //сбор расширений
                if (lineItemExtension.getFieldName() != null && lineItemExtension.getFieldName().equals("DATAMATRIX")) {
                    String fieldValue = lineItemExtension.getFieldValue();
                    if (fieldValue != null) {
                        lineItemExtDataMatrixMap.put(lineItemExtension.createKeyLineItem(), fieldValue);
                    }
                }
        ...
        for (Map.Entry<String, List<RetailLineItem>> retailLineItemCancelEntry : retailLineItemsCancel.entrySet()) {
            ...
            List<RetailLineItem> notCancelList = retailLineItemsNotCancel.getOrDefault(key, Collections.emptyList());
            for (RetailLineItem retailLineItemCancel : retailLineItemsCancelList) {
                ...
                boolean flAlc = false;
                boolean flCis = false;
                if (alcMarkDel != null && !alcMarkDel.isEmpty()) {   //замена типа позиции продажи по номеру алкомарки (ABAP стр. 134-158)
                    flAlc = updateRetailTypeCodeByMark(notCancelList, retailLineItemCancel,
                            lineItemExtTaxMap, qty, amnt, alcMarkDel);
                }
                if (!flAlc && cisMarkDel != null && !cisMarkDel.isEmpty()) {   //замена типа позиции продажи по номеру КИЗ
                    flCis = updateRetailTypeCodeByMark(notCancelList, retailLineItemCancel,
                            lineItemExtDataMatrixMap, qty, amnt, cisMarkDel);
                }
                if (!flAlc && !flCis) { //Товар не алкогольный или же не найден нужный номер алкомарки
                    updateRetailTypeCodeByMark(notCancelList, retailLineItemCancel,
    ...
    private boolean updateRetailTypeCodeByMark(List<RetailLineItem> retailLineItemsNotCancel, RetailLineItem retailLineItemCancel,
        ...
        if (retailLineItemsNotCancel == null || retailLineItemsNotCancel.isEmpty()) {
            return false;
        }
    ...
    private void addToMapLineItemExtensionTax(Map<String, String> lineItemExtensions, LineItemExtension lineItemExtension) {
        ...
        if (key == null || value == null || value.isEmpty()) {
            return;  //пропуск пустых TAX-значений и записей без ключа (аналог ABAP: CONCATENATE с пустым значением не меняет результат)
        }
