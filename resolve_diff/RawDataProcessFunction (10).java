...
public class RawDataProcessFunction extends KeyedProcessFunction<String, String, RowData> {
    ...
    public void processElement(String kafkaValue, Context context, Collector<RowData> out){
        ...
        try (InputStream inputStream = CustomDecoder.decodeAndDecompress(kafkaValue)) {
            ...
            Set<String> autoMDSet = chkFixesHas2125781 ? getAutoMDSet(corrBaseTransactionKeyList) : Collections.emptySet();   //получение автоуценок только если есть запись в CHK_FIXES
                ...
                try {
                    ...
                    if (baseTransactionKey instanceof RetailLineItem) {
                        ...
                        if (pstSchema != null) {
                            ...
                            } else if (retailLineItem.getHasEan() == null || Boolean.TRUE.equals(retailLineItem.getHasEan())) {  //если ean найден или нет маппинга
